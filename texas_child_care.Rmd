...
# compare number of fully licensed centers in Feb. 1, 2020 to Nov. 27, 2021
child_care_centers %>% 
  filter(Permit_Status == 'Full', NM_COUNTY != '') %>% 
  group_by(NM_COUNTY, Week) %>% 
  summarize(total_capacity = sum(NBR_TOTAL_CPCTY, na.rm = T)) %>% 
  merge(., children_by_county %>% 
          filter(county != 'Texas') %>% 
          mutate(county = toupper(county)) %>% 
          rename(NM_COUNTY = county)) %>% 
  summarize(Week, NM_COUNTY, under_5 = under_3_years + X3_to_4_years + X5_years, 
            total_capacity, ratio = under_5 / total_capacity) %>% 
  arrange(NM_COUNTY, Week) %>% 
  filter(Week == as.Date('2021-11-27') | Week == as.Date('2020-02-01')) %>% 
  select(Week, NM_COUNTY, ratio) %>% 
  pivot_wider(names_from = Week, values_from = ratio) %>% 
  #filter(`2020-02-01` < 3 & `2021-11-27` >= 3) %>% 
  filter(`2020-02-01` >= 3 & `2021-11-27` > `2020-02-01`)
```
...
children_by_county %>% 
  ...
  filter(county %in% setdiff(toupper(children_by_county$county), 
                             filter(before_after_schools, Week == as.Date('2021-11-27'))$NM_COUNTY)) %>% 
  filter(county != 'TEXAS')

# count number of centers fully licensed for before/after school care statewide per month
before_after_schools %>% 
  filter(day(Week) == 1) %>% 
  count(Week)
...
# count number of fully licensed centers providing before/after school care statewide per month
before_after_schools %>% 
  filter(day(Week) == 1) %>% 
  group_by(Week) %>% 
  count(Operating_Status) %>% 
...
child_care_centers %>% 
  filter(Permit_Status == 'Full', Operating_Status == 'Y', Week == as.Date('2021-11-27')) %>% 
  filter(NM_COUNTY %in% child_care_deserts$COUNTY) # to find still open centers in deserts

# use 2019 for benchmark numbers
# count average number of fully licensed centers providing care statewide per month
child_care_centers %>% 
  filter(Permit_Status == 'Full') %>% 
  group_by(Week) %>% 
  count(Operating_Status) %>% 
  ...
  pivot_wider(names_from = `year(Week)`, values_from = mean)

# chart number of fully licensed centers providing care statewide per week
child_care_centers %>% 
  filter(Permit_Status == 'Full') %>% 
  group_by(Week) %>% 
  count(Operating_Status) %>% 
...
# chart average number of fully licensed centers providing before/after school care statewide per month
before_after_schools %>% 
  group_by(Week) %>% 
  count(Operating_Status) %>% 
  filter(Operating_Status == 'Y') %>% 
  group_by(year(Week), month(Week)) %>% 
  summarize(date = mdy(paste0(as.character(`month(Week)`), '/01/', `year(Week)`)),
            mean = mean(n)) %>% 
  ggplot(data = ., mapping = aes(x = date, y = mean)) + 
  geom_line() +
  theme_classic()
```
...
# chart number of centers fully licensed for before/after school care from January 2019 to November 2021
before_after_schools %>% 
  filter(day(Week) == 1) %>% 
  count(Week) %>% 
  ggplot(data = ., mapping = aes(x = Week, y = n)) +
...
# chart percent of fully licensed centers providing before/after school care from January 2019 to November 2021
child_care_centers %>% 
