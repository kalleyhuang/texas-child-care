...
child_care_operations %>% 
  ...
  ungroup() %>% 
  mutate(diff = as.integer(Week - as.Date('2019-01-05'))) %>% # remove dates not in weekly time series
  filter(diff %% 7 == 0) %>% # remove dates not in weekly time series
  select(-diff) %>% 
  mutate(pct_n = n / 28612 * 100)
...
child_care_operations %>% 
  ...
  merge(., child_care_operations %>% 
           ...
           filter(NM_COUNTY != '') %>% 
           rename(total = n)) %>% 
  arrange(NM_COUNTY, `year(Week)`, `month(Week)`) %>% 
  mutate(pct_not = n / total * 100) %>% 
  select(`month(Week)`, `year(Week)`, NM_COUNTY, n, total, pct_not)

# count types of centers "not caring" as of November 2021 and calculate their median capacity
child_care_operations %>% 
  filter(Operating_Status == 'N', Week == as.Date('2021-11-27')) %>% 
  group_by(OP_TYPE) %>% 
...
child_care_operations %>% 
  filter(Week == as.Date('2021-11-27'), Operating_Status == 'Y', NM_COUNTY != '') %>% 
  group_by(NM_COUNTY) %>% 
...
child_care_operations %>% 
  filter(OP_TYPE == 'Licensed Center - Before/After School Program') %>% 
  filter(month(Week) == 9, Operating_Status == 'N') %>% 
  count(NM_COUNTY) %>% 
  arrange(desc(n))
```
...
child_care_operations %>% 
  ...
  count(Operating_Status) %>% 
  filter(Operating_Status == 'N') %>% 
  # mutate(month = as.character(`month(Week)`), year = as.character(`year(Week)`),
