...
# from https://censusreporter.org/data/table/?table=B09001&geo_ids=04000US48,050|04000US48&primary_geo_id=04000US48#
...
# count number of child care licenses issued from 1983 to 2021 (through Sept. 27)
child_care %>% 
  count(year(ISSUANCE_DATE))

# count number of child care licenses issued to each county in 2019, 2020, 2021
child_care %>% 
...
# count number of centers "not caring" statewide per week from January 2021 to May 2021
child_care_operations_2021 %>% 
  group_by(Week) %>% 
  count(Status) %>% 
...
# count types of centers "not caring" as of May 2021 and calculate their median capacity
child_care_operations_2021 %>% 
...
child_care %>% 
  filter(TYPE_OF_ISSUANCE == 'Full Permit') %>% 
  group_by(COUNTY) %>% 
...
# calculate ratio of children under 5 to total capacity of centers open as of May 1, 2021, by county
child_care_operations_2021 %>% 
  filter(Week == as.Date('2021-05-01'), Status == 'Y', County != '') %>% 
  group_by(County) %>% 
...
child_care %>% 
  ...
  geom_line() +
  labs(y = 'number of child care licenses issued') +
  theme_classic()

# visualize number of centers "not caring" from January 2021 to May 2021
child_care_operations_2021 %>% 
  filter(day(Week) == 1) %>% 
  group_by(Week) %>% 
  count(Status) %>% 
...
child_care_operations_2021 %>% 
  filter(day(Week) == 1, County %in% c('TARRANT', 'TRAVIS', 'EL PASO', 'WEBB')) %>% 
  group_by(Week, County) %>% 
  count(Status) %>% 
  filter(Status == 'N') %>% 
  select(-Status) %>% 
...
all %>% 
  filter(area == 48) %>% 
  select(year, occ_title, h_median) %>% 
  pivot_wider(names_from = occ_title, values_from = h_median) %>% 
  select(-3, -8)
```
