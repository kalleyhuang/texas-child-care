...
child_care_2021_06$Operation.Region <- as.character(child_care_2021_06$Operation.Region) # reformat to bind later
...
child_care_2021_09 <- read.csv('child_care_operations_2021_09.csv')
...
child_care_permits_2021 <- child_care_2021_01 %>% 
  ...
  bind_rows(., child_care_2021_02 %>% 
              ...
              pivot_longer(cols = starts_with('Permit.Status.'), names_to = 'Week', 
                           names_prefix = 'Permit.Status.', values_to = 'Status') %>% 
              select(1:11, Week, Status), child_care_2021_09 %>% 
              pivot_longer(cols = starts_with('Permit.Status.'), names_to = 'Week', 
                           names_prefix = 'Permit.Status.', values_to = 'Status')) %>% 
...
# count number of operations by type in most recent data for operations status
child_care_operations_2021 %>% 
  filter(Week == as.Date('2021-09-01')) %>% 
...
# count number of centers "not caring" and not in licensing data in each county
child_care_operations_2021 %>%
  filter(Week == as.Date('2021-09-01')) %>% 
  mutate(Operation.Number = gsub('-|- | ', '', Operation.Number)) %>% 
  anti_join(., child_care %>% 
...
children_by_county %>% 
  ...
  filter(county %in% setdiff(toupper(children_by_county$county), child_care$COUNTY)) %>% 
...
child_care_operations_2021 %>% 
  group_by(month(Week), County) %>% 
  count(Status) %>% 
  filter(Status == 'N') %>% 
  merge(., child_care_operations_2021 %>% 
          filter(Week == as.Date('2021-09-01')) %>% 
          count(County) %>% 
          filter(County != '') %>% 
          rename(total = n)) %>% 
...
# count types of centers "not caring" as of September 2021 and calculate their median capacity
child_care_operations_2021 %>% 
  filter(Status == 'N', Week %in% as.Date('2021-09-01')) %>% 
  group_by(Operation.Type) %>% 
  summarize(Capacity = median(Capacity, na.rm = T)) %>% 
  bind_cols(., child_care_operations_2021 %>% 
              filter(Status == 'N', Week == as.Date('2021-09-01')) %>% 
              count(Operation.Type) %>% 
              select(n)) %>% 
  ungroup() %>% 
...
# calculate ratio of children under 5 to total capacity of centers caring as of Sept. 1 for each county
child_care_operations_2021 %>% 
  filter(Week == as.Date('2021-09-01'), Status == 'Y', County != '') %>% 
  group_by(County) %>% 
  ...
  merge(., children_by_county %>% 
          ...
          rename(County = county)) %>% 
  summarize(County, under_5 = under_3_years + X3_to_4_years + X5_years, 
            Capacity, ratio = under_5 / Capacity) %>% 
  arrange(desc(ratio)) %>% 
  filter(ratio >= 3)
```
...
before_after_schools <- child_care_operations_2021 %>% 
  filter(Operation.Type == 'Licensed Center - Before/After School Program') %>% 
  select(-Week, -Status) %>% 
  group_by(Operation.Number) %>% 
...
children_by_county %>% 
  mutate(county = toupper(county)) %>% 
  filter(county %in% setdiff(toupper(children_by_county$county), before_after_schools$County)) %>% 
  filter(county != 'TEXAS')
...
multiple_not_caring_2021 <- child_care_operations_2021 %>% 
  ...
  select(Operation.Name, Address, Week, Status, Operation.Type, Capacity, 
         ADMINISTRATOR_DIRECTOR_NAME, PHONE_NUMBER, email_address, WEBSITE_ADDRESS, 
         Permit.Issue.Date, PROGRAMS_PROVIDED, LICENSED_TO_SERVE_AGES) %>% 
  filter(Status == 'N')
...
not_caring_2021 <- child_care_operations_2021 %>% 
  ...
  left_join(., child_care, by = c('Operation.Number' = 'OPERATION_NUMBER')) %>% 
  mutate(Address = paste0(Address.Line.1, ', ', City, ', TX ', Zip.Code)) %>% 
  select(Operation.Name, Address, Week, Operation.Type, Capacity, 
         ADMINISTRATOR_DIRECTOR_NAME, PHONE_NUMBER, email_address, WEBSITE_ADDRESS, 
         Permit.Issue.Date, PROGRAMS_PROVIDED, LICENSED_TO_SERVE_AGES)

# to export spreadsheet of centers "not caring" and not in licensing data
not_caring_not_licensed <- child_care_operations_2021 %>% 
  filter(Week == as.Date('2021-09-01')) %>% 
  mutate(Operation.Number = gsub('-|- | ', '', Operation.Number)) %>% 
  anti_join(., child_care %>% 
              mutate(OPERATION_NUMBER = gsub('-|- | ', '', OPERATION_NUMBER)),
...
# visualize number of centers "not caring" from January 2021 to September 2021
child_care_operations_2021 %>% 
  group_by(month(Week)) %>% 
  count(Status) %>% 
  filter(Status == 'N') %>% 
