...
child_care_centers_2021_12 <- bind_cols(child_care_operations_2021_12, child_care_permits_2021_12 %>% 
  ...
  filter(OP_NBR != '')
child_care_centers <- child_care_centers %>% 
  bind_rows(., child_care_centers_2021_12) %>% 
  arrange(OP_NBR, Week)
```
...
# compare to percent of fully licensed centers providing child care as of December 2021
child_care_centers %>% 
  filter(Permit_Status == 'Full', Week == as.Date('2021-12-01') | Week == as.Date('2020-02-01')) %>% 
  group_by(Week, NM_COUNTY) %>% 
  count(Operating_Status) %>% 
  mutate(pct_caring = n / sum(n) * 100) %>% 
  filter(Operating_Status == 'Y') %>% 
  ...
  filter(diff < 0) %>% 
  merge(., child_care_centers %>% 
          filter(Permit_Status == 'Full', Week == as.Date('2021-12-01')) %>% 
          count(NM_COUNTY) %>% 
          rename(total = n)) %>% 
  mutate(desert = NM_COUNTY %in% child_care_deserts$COUNTY) %>% 
  arrange(diff)

# count types of fully licensed centers "not caring" as of December 2021 and calculate their median capacity
child_care_centers %>% 
  filter(Operating_Status == 'N', Permit_Status == 'Full', Week == as.Date('2021-12-01')) %>% 
  count(OP_TYPE) %>% 
  merge(., child_care_centers %>% 
    filter(Operating_Status == 'N', Permit_Status == 'Full', Week == as.Date('2021-12-01')) %>% 
    group_by(OP_TYPE) %>% 
...
child_care_centers %>% 
  ...
  filter(Week == as.Date('2021-12-01')) %>% 
  merge(., child_care_centers %>% 
          filter(NM_COUNTY != '', Week == as.Date('2021-12-01')) %>% 
...
children_by_county %>% 
  ...
  filter(county %in% setdiff(toupper(children_by_county$county), 
                             filter(before_after_schools, Week == as.Date('2021-12-01'))$NM_COUNTY)) %>% 
  filter(county != 'TEXAS')
...
child_care_centers %>% 
  filter(Permit_Status == 'Full', Operating_Status == 'Y', Week == as.Date('2021-12-01')) %>% 
